...
import { Search, Download, Visibility, CheckCircle, Settings } from '@mui/icons-material';
import {
...
const AdminOrders = () => {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      ...
      // Stock changes handled by notifyUserOnStatusChange Cloud Function.
  ...
  // Fix 4: Override — just write the status. Cloud Function handles stock.
  const handleOverrideStatus = async () => {
    if (!overrideDialog || !overrideNewStatus) return;
    if (overrideNewStatus === overrideDialog.status) {
      setOverrideError('Please select a different status.');
      return;
    }
    ...
    try {
      const ref = doc(db, COLLECTIONS.ORDERS, overrideDialog.id);
      const freshSnap = await getDoc(ref);
      ...
      await updateDoc(ref, {
        ...
        statusHistory: [
          ...freshHistory,
          {
            ...
            note:      `Admin override from ${overrideDialog.status}`,
          },
        ],
        updatedAt: serverTimestamp(),
      });

      // notifyUserOnStatusChange Cloud Function fires on this write and handles:
      //   cancelled → delivered : deduct stock
      //   delivered → cancelled : restock
      //   any       → cancelled : restock (with double-cancel guard)

      setOrders((prev) =>
        prev.map((o) => o.id === overrideDialog.id ? { ...o, status: overrideNewStatus } : o)
      );
      setSuccessMsg(`Order status overridden to "${overrideNewStatus}"`);
      setTimeout(() => setSuccessMsg(''), 4000);
      setOverrideDialog(null);
      setOverrideNewStatus('');
    } catch (err) {
      console.error('handleOverrideStatus error:', err);
      setOverrideError(`Failed: ${err.message}`);
    } finally {
      setOverriding(false);
    }
  };
    ...
    <Box sx={{ p: { xs: 2, sm: 3 } }}>
      ...
      <TableContainer component={Paper} elevation={0} sx={{ border: `1px solid ${ZAP_COLORS.border}`, borderRadius: 3 }}>
        <Table size="small">
          ...
          <TableBody>
            {loading ? (
              <TableRow><TableCell colSpan={8} sx={{ textAlign: 'center', py: 4 }}><CircularProgress size={24} sx={{ color: ZAP_COLORS.primary }} /></TableCell></TableRow>
            ) : filteredOrders.length === 0 ? (
              <TableRow><TableCell colSpan={8} sx={{ textAlign: 'center', py: 4, color: ZAP_COLORS.textMuted }}>No orders found</TableCell></TableRow>
              ...
              return (
                <TableRow key={order.id} hover>
                  <TableCell sx={{ fontSize: '0.78rem', fontWeight: 700 }}>
                    #{order.orderNumber}
                    {order.totalAdjusted && (
                      <Chip label="⚠️ Adjusted" size="small" sx={{ ml: 0.5, fontSize: '0.6rem', height: 16, background: '#FFF3E0', color: '#E65100' }} />
                    )}
                  </TableCell>
                  ...
                  <TableCell sx={{ fontSize: '0.78rem', fontWeight: 700, color: ZAP_COLORS.primary }}>₹{order.total}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                      ...
                      {isTerminal && (
                        <IconButton size="small" title="Override status"
                          onClick={() => { setOverrideDialog(order); setOverrideNewStatus(''); setOverrideError(''); }}
                          sx={{ color: ZAP_COLORS.textSecondary, '&:hover': { color: ZAP_COLORS.primary, background: `${ZAP_COLORS.primary}10` } }}>
      ...
      {totalPages > 1 && (
        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2, gap: 1 }}>
          <Button disabled={page === 0} onClick={() => fetchOrders(page - 1, statusFilter)} variant="outlined" size="small">Prev</Button>
          <Typography variant="body2" sx={{ alignSelf: 'center' }}>Page {page + 1} / {totalPages}</Typography>
          <Button disabled={page + 1 >= totalPages} onClick={() => fetchOrders(page + 1, statusFilter)} variant="outlined" size="small">Next</Button>
        </Box>
      )}

      {/* Order detail dialog */}
      <Dialog open={!!detailOrder} onClose={() => setDetailOrder(null)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
        <DialogTitle fontWeight={700} sx={{ fontFamily: "'Syne', sans-serif" }}>Order #{detailOrder?.orderNumber}</DialogTitle>
        <DialogContent>
          {detailOrder && (
            <Box>
              <Box sx={{ mb: 2 }}>
                <Typography variant="caption" color="text.secondary" display="block">Customer</Typography>
                <Typography variant="body2" fontWeight={600}>{detailOrder.customerName}</Typography>
                <Typography variant="body2">{detailOrder.customerEmail}</Typography>
                {detailOrder.customerPhone && <Typography variant="body2">{detailOrder.customerPhone}</Typography>}
              </Box>
              {detailOrder.address && (
                <Box sx={{ mb: 2 }}>
                  <Typography variant="caption" color="text.secondary" display="block">Delivery Address</Typography>
                  <Typography variant="body2">
                    {detailOrder.address.line1}{detailOrder.address.line2 ? `, ${detailOrder.address.line2}` : ''}, {detailOrder.address.city} - {detailOrder.address.pincode}
                  </Typography>
                </Box>
              )}
              {detailOrder.totalAdjusted && (
                <Alert severity="warning" sx={{ mb: 2, borderRadius: 2, fontSize: '0.8rem' }}>Original client-submitted total did not match product prices.</Alert>
              )}
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" onClick={() => generateInvoicePDF(detailOrder, null)} startIcon={<Download />}>Invoice</Button>
        </DialogActions>
      </Dialog>

      {/* Fix 4: Override status dialog */}
      <Dialog open={!!overrideDialog} onClose={() => !overriding && setOverrideDialog(null)}
        maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
        <DialogTitle fontWeight={700} sx={{ fontFamily: "'Syne', sans-serif" }}>⚙️ Override Order Status</DialogTitle>
        <DialogContent>
          {overrideError && <Alert severity="error" sx={{ mb: 2, borderRadius: 2 }}>{overrideError}</Alert>}
          <Alert severity="warning" sx={{ mb: 2, borderRadius: 2 }}>
            Override order <strong>#{overrideDialog?.orderNumber}</strong>. Stock will be adjusted automatically by the server where needed.
          </Alert>
          <Typography variant="body2" color="text.secondary" mb={0.5}>
            Current status: <strong>{overrideDialog?.status}</strong>
          </Typography>
          <Typography variant="body2" fontWeight={600} mb={1} mt={1.5}>Change to:</Typography>
          <FormControl fullWidth size="small">
            <Select value={overrideNewStatus} onChange={(e) => setOverrideNewStatus(e.target.value)} displayEmpty>
              <MenuItem value="" disabled>Select new status…</MenuItem>
              {ALL_STATUS_OPTIONS.filter((s) => s.key !== overrideDialog?.status).map((s) => (
                <MenuItem key={s.key} value={s.key} sx={{ fontSize: '0.85rem' }}>{s.icon} {s.label}</MenuItem>
              ))}
            </Select>
          </FormControl>
          {overrideNewStatus && overrideDialog && (() => {
            const from = overrideDialog.status;
            const to   = overrideNewStatus;
            if (from === 'cancelled' && to === 'delivered')
              return <Alert severity="info" sx={{ mt: 1.5, borderRadius: 2, fontSize: '0.78rem' }}>📦 Server will <strong>deduct stock</strong> (restocked when cancelled).</Alert>;
            if (from === 'delivered' && to === 'cancelled')
              return <Alert severity="info" sx={{ mt: 1.5, borderRadius: 2, fontSize: '0.78rem' }}>♻️ Server will <strong>restock items</strong> (previously marked delivered).</Alert>;
            return null;
          })()}
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          <Button onClick={() => { setOverrideDialog(null); setOverrideNewStatus(''); setOverrideError(''); }} disabled={overriding} variant="outlined" fullWidth>Cancel</Button>
          <Button onClick={handleOverrideStatus} disabled={overriding || !overrideNewStatus} variant="contained" color="warning" fullWidth>
            {overriding ? <CircularProgress size={18} sx={{ color: '#fff' }} /> : 'Confirm Override'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
