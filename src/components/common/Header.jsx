...
const Header = () => {
  ...
  return (
    <>
      <HideOnScroll>
        <AppBar position="sticky" elevation={0}>
          <Toolbar sx={{ px: { xs: 1.5, sm: 2 }, minHeight: { xs: 56, sm: 64 }, gap: 1 }}>
            ...
            <Box onClick={() => setLocationDialogOpen(true)} sx={{ display: 'flex', alignItems: 'center', gap: 0.3, px: 1, py: 0.5, borderRadius: 2, background: `${ZAP_COLORS.primary}10`, cursor: 'pointer', flex: 1, maxWidth: 200, overflow: 'hidden', border: `1px solid ${ZAP_COLORS.primary}20`, '&:hover': { background: `${ZAP_COLORS.primary}18` }, '&:active': { transform: 'scale(0.97)' } }}>
            ...
            <IconButton size="small" onClick={() => setSearchOpen(true)} sx={{ color: ZAP_COLORS.textSecondary }}><Search /></IconButton>
            ...
            {user ? (
              ...
              <Button size="small" variant="contained" onClick={() => navigate('/login', { state: { from: location } })} sx={{ minWidth: 0, px: 1.5, py: 0.5, fontSize: '0.78rem' }}>Login</Button>
            )}
          </Toolbar>
            ...
            <Box sx={{ px: 2, py: 0.5, background: `linear-gradient(90deg, ${ZAP_COLORS.primary}12, ${ZAP_COLORS.accent}10)`, borderTop: `1px solid ${ZAP_COLORS.primary}15`, display: 'flex', alignItems: 'center', gap: 0.5 }}>
              ...
              {activeUserStore.distanceKm !== undefined && (
                <Typography variant="caption" sx={{ color: ZAP_COLORS.textMuted, fontSize: '0.65rem', ml: 0.5 }}>
                  · {activeUserStore.distanceKm < 1 ? `${(activeUserStore.distanceKm * 1000).toFixed(0)}m away` : `${activeUserStore.distanceKm.toFixed(1)}km away`}
                </Typography>
              )}
            </Box>
          )}
      ...
      {searchOpen && (
        <Box sx={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(26,26,46,0.7)', backdropFilter: 'blur(8px)', zIndex: 1400, display: 'flex', flexDirection: 'column', alignItems: 'center', pt: { xs: 4, sm: 8 }, px: 2 }} onClick={(e) => e.target === e.currentTarget && setSearchOpen(false)}>
          <Paper elevation={8} sx={{ width: '100%', maxWidth: 600, borderRadius: 3, overflow: 'hidden' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', px: 2, py: 1.5 }}>
              <Search sx={{ color: ZAP_COLORS.primary, mr: 1.5 }} />
              <InputBase autoFocus fullWidth placeholder="Search for products, categories..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={handleSearch} sx={{ fontSize: '1rem', fontFamily: "'DM Sans', sans-serif" }} />
              <IconButton size="small" onClick={() => setSearchOpen(false)}><Close fontSize="small" /></IconButton>
            </Box>
          </Paper>
          <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.6)', mt: 2 }}>Press Enter to search</Typography>
        </Box>
      )}
